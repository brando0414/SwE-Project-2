...
def predict(input_image, output_image, conf=40, ovl=30):
    # Make predictions
    predictions = model.predict(input_image, confidence=conf, overlap=ovl).json()
...
for file_name in os.listdir(folder_path):
    ...
    if os.path.isfile(file_path):  # Check if it's a file
        ...
        predict(folder_path + "/" + file_name,
                "predicted_output_images/" + f'prediction_' + f'{file_num:05d}.jpg', 40, 30)

# convert those predicted images in the predicted_output_images directory back into a video (mp4)
images_to_video("predicted_output_images", "predicted_output_video.mp4", 30)
